...
/*
Generate a list of all folders in the Filesystem tree.
*/
void listFolders(FSNode* node, std::vector<FSNode*>* folderList) {
    if(node->type == Folder) {
        for(int i = 0; i < node->contents.size(); i++) {
            listFolders(&node->contents.at(i), folderList);
        }
...
/*
Compare two pointers to nodes by node size.
Used in conjunction with sorting function to order list of nodes by size.
*/
bool compareNodePtr(FSNode* a, FSNode* b) {
...
int main() {
    ...
    */
    ...
    /*
    Create and populate a list of pointers to folder nodes
    */
    std::vector<FSNode*> folders;
    listFolders(&root, &folders);

    /*
    Sort the list of folders so they are in ascending order.
    */
    std::sort(folders.begin(), folders.end(), compareNodePtr);


    /*
    Find the smallest folder that will free enough space on the fs
    */
    int unusedSpace = capacity - root.size;
    for(int i = 0; i < folders.size(); i++) {
        if(folders.at(i)->size + unusedSpace > spaceNeeded) {
            printf("%d\n", folders.at(i)->size);
            break;
        }
